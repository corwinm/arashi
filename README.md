...
You can also build from source for local development. Development is pinned to Node.js 24.18.0, which satisfies pnpm 11's Node.js 22.13 minimum, and pnpm 11.22.0; nvm users can select the repository's development runtime with `nvm use`.
...
#### Optional repository worktree setup

In an interactive TTY, `aw add` offers optional repository worktree setup after materialization and defaults to **No**. A top-level decline completes the existing minimal add. Non-TTY input/output, `--json`, and `--force` suppress discovery and onboarding entirely.

Accepted setup is limited to repository `copy` and `symlink` paths plus `pre-create`, `post-create`, `pre-remove`, and `post-remove` hooks. Hook sources are exclusive: user-supplied inline Bash/interpreter-map commands or a generated active file. Inline command bodies and generated file contents are never shown; summaries identify only lifecycle/interpreter presence or file path/state. Generated files are active immediately, executable-ready, safe no-op scaffolds ready for editing. Create-hook files live under the active configuration root; remove-hook files live in the runtime-resolved target repository.

Arashi validates one isolated complete candidate, asks once more, then installs files and saves configuration once. Final decline or Ctrl+C uses add rollback. Installation uses metadata-only validation, symlink rejection, private complete-file preparation, and atomic no-replace publication. Pure Node/Bun path APIs still leave a narrow residual local race if another process with workspace write access substitutes an ancestor between final validation and publication.
