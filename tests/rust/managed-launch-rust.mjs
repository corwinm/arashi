// Rust execution of the retained 19-case transcript; fixtures are NOT vendor acceptance.
// Run: MANAGED_TEST_BIN=/absolute/test-binary node tests/rust/managed-launch-rust.mjs [report.json]
...
const testBinary = process.env.MANAGED_TEST_BIN;
assert.ok(testBinary, "MANAGED_TEST_BIN must identify the compiled Rust integration test");
try {
  ...
  function check(name, options, env, responses, expected, candidateOverride = {}) {
    ...
    const child = spawnSync(
      testBinary,
      ["--ignored", "--exact", "native::fixture_entry", "--nocapture"],
      {
        cwd,
        encoding: "utf8",
        env: {
    ...
    const line = child.stdout.split("\n").find((line) => line.startsWith("ARASHI_FIXTURE_RESULT="));
    assert.ok(line, child.stdout);
    const output = JSON.parse(line.slice("ARASHI_FIXTURE_RESULT=".length));
  ...
  const report = {
    kind: "Rust subprocess protocol fixtures from retained source, not live vendor acceptance",
