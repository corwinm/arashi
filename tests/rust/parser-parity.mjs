...
const contract = JSON.parse(
  readFileSync(new URL("../../contracts/cli-commands.json", import.meta.url)),
);
...
const run = (sourceMode, args) => {
  const p = spawnSync(
    sourceMode ? process.execPath : binary,
    sourceMode ? [source, ...args] : args,
...
try {
  ...
  if (process.argv.includes("--capture-help")) {
    writeFileSync(
      new URL("../../src/rust/parser-help.json", import.meta.url),
      JSON.stringify(helps, null, 2) + "\n",
    );
  }
  const cases = [
    ...Object.keys(helps).flatMap((c) => [
      [...c.split(" ").filter(Boolean), "--help"],
      ["help", ...c.split(" ").filter(Boolean)],
    ]),
    [],
    ["shell"],
    ["help", "absent"],
    ...
    ["install", "one", "two", "-j"],
    ["shell", "absent"],
    ["shell", "init", "invalid", "-h"],
    ["shell", "init", "bash", "extra", "-h"],
    ["list", "--max-depth"],
    ["create", "--base"],
    ["create", "--conflict", "bad"],
    ["init", "--ignore-scope", "bad"],
    ["list", "--json=true"],
    ["list", "-jx"],
    ["list", "--absent", "--help"],
    ["list", "--max-depth", "--help"],
    ...
    ["status", "-jsv"],
    ["status", "-jj"],
    ["status", "-j", "-oone", "--only", "two"],
    ["status", "--only=-foo", "-j"],
    ["create", "--no-launch", "--launch", "-h"],
    ["create", "--launch", "--no-launch", "-h"],
    ...["bash", "zsh", "fish"].flatMap((shell) => [
      ["shell", "init", shell],
    ...
    ...[
      ...
      ["--sesh", "--herdr"],
      ["--interactive"],
    ].map((flags) => ["create", "feature", "-j", "--no-hooks", ...flags]),
    ["exec", "node", "--eval", "console.log(1)", "--", "--help"],
    ["list", "--absent", "--", "--help"],
    ["shell", "help", "init"],
    ["shell", "--help", "init"],
    ["completion", "invalid"],
    ...
    ["status", "--jsno"],
    ["shell", "instal"],
    ...["1e2", "1.0", "+1", " 1", "", "9007199254740992", "0003"].map((v) => [
      "list",
      `--max-depth=${v}`,
      "-h",
    ]),
