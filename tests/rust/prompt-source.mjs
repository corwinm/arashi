...
let result, expected;
const semantic = process.env.ARASHI_PROMPT_SEMANTIC && JSON.parse(process.env.ARASHI_PROMPT_SEMANTIC);
if (semantic) {
  result = await p[semantic.kind](semantic.kind === 'input' ? 'Enter text' : 'Proceed', semantic.default ?? undefined);
