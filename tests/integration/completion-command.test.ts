...
if [[ "$*" == "completion __query 3 -- arashi switch --path C:/work" ]]; then
  ...
  printf 'repo=v2\\0Repository\\0'
elif [[ "$*" == "completion __query 4 -- arashi create topic --only repo o" ]]; then
  printf 'repo one\\0Repository\\0'
fi
`,
        );
