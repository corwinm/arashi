...
async function createWorkspace(baseDir: string, options: WorkspaceOptions = {}): Promise<{ workspaceRoot: string; repoAPath: string; repoBPath: string }> {
  ...
  const config: {
    ...
    discovered_repos: Record<string, { path: string; default_branch: string; is_bare: boolean; worktrees: never[] }>;
  } = {
    version: '1.0.0',
    repos_dir: './repos',
    auto_setup: true,
