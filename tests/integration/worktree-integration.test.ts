...
import type { RepositoryFilter, WorktreeOperationOptions } from "../../src/core/worktree.ts";
...
describe("Basic Coordinated Worktree Creation (Integration)", () => {
  test("should create worktrees across 3 real test repositories", async () => {
    ...
    for (const repoResult of result.repositoryResults) {
      ...
      const branchExists = await verifyBranchExists(
        repoResult.repository.path,
        branchName
      );
...
describe("Rollback on Repository Failure (Integration)", () => {
  test("should rollback first 2 worktrees when 3rd repository fails", async () => {
    ...
    const branch1Exists = await verifyBranchExists(
      workspace.repositories[0].path,
      branchName
    );
    const branch2Exists = await verifyBranchExists(
      workspace.repositories[1].path,
      branchName
    );
...
describe("Conflict Detection and Resolution (Integration)", () => {
  test("should detect and handle branch conflicts with REUSE_EXISTING strategy", async () => {
    ...
    const { createCoordinatedWorktrees, checkBranchConflicts } = await import("../../src/core/worktree.ts");
    
    // Check for conflicts
    const conflictCheck = await checkBranchConflicts(
      existingBranchName,
      workspace.repositories
    );
