...
async function createUniqueWorktree(mainRepoPath: string, branchName: string): Promise<string> {
  const worktreePath = join(mainRepoPath, '..', `worktree-${branchName}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`);
 ...
 */
async function createWorktree(mainRepoPath: string, branchName: string, worktreePath: string): Promise<void> {
 ...
 */
async function runListCommand(cwd: string, options?: { verbose?: boolean; json?: boolean; table?: boolean; maxDepth?: number }): Promise<{
  output: string;
  error?: Error;
}> {
...
describe('list command - JSON output', () => {
  ...
  test('JSON output includes all worktrees', async () => {
    ...
    const paths = parsed.map((wt: any) => realpathSync(wt.path));
...
describe('list command - locked worktrees', () => {
  ...
  test('JSON output includes locked status', async () => {
    ...
    const lockedWorktree = parsed.find((wt: any) => realpathSync(wt.path) === canonicalWtPath);
...
describe('list command - detached HEAD', () => {
  ...
  test('shows detached HEAD correctly', async () => {
    ...
    const wtPath = join(testDir, '..', `worktree-detached-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`);
  ...
  test('JSON shows null branch for detached HEAD', async () => {
    ...
    const wtPath = join(testDir, '..', `worktree-detached-json-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`);
    await spawn(['git', 'worktree', 'add', '--detach', wtPath, commit], {
      cwd: testDir,
    }).exited;
    ...
    const { realpathSync } = await import('fs');
    const canonicalWtPath = realpathSync(wtPath);
    const detachedWorktree = parsed.find((wt: any) => realpathSync(wt.path) === canonicalWtPath);
