...
This support describes implemented behavior, not an invitation to install an arbitrary branch build. Before a tester handoff, the exact candidate head still needs the integrated Windows cleanup lane, complete Rust and alpha gates on every workflow platform, installation and representative dogfooding from the retained artifact, refresh plus rollback/uninstall verification, and pushed-SHA CI readback. Until those gates pass, build and test only from a reviewed local checkout; do not treat an unverified workflow artifact as a release.
