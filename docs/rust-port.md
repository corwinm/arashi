...
Exec/setup preflight all configured paths before starting children. Nonempty materialization source projection, external/traversing/symlinked paths and duplicate main/child names fail explicitly. Windows exec `.cmd`/`.bat` argv transport remains rejected. Bare/linked execution projection, malformed configuration/error precedence, concurrent fail-fast races and broader launch-error/platform cases remain unclaimed. The shared native process runner is groundwork for lifecycle hooks, not an implementation of hook discovery, provenance, inline interpreters, terminal input, interruption or recovery. Create/remove/doctor hook guards remain unchanged.
