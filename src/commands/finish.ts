...
export async function assessFinish(
  ...
  const common = await git(parentPath, "rev-parse", "--git-common-dir");
  ...
  for (const item of entries) {
    ...
    try {
      ...
      repo.head = await git(item.path, "rev-parse", "HEAD");
      ...
      try {
        ...
        if (
          ...
          try {
            const upstreamUrl = await git(item.source, "remote", "get-url", upstreamRemote);
            await git(
              privateRepo,
              "fetch",
              "--no-tags",
            ...
            const upstreamOid = await git(privateRepo, "rev-parse", "refs/finish/upstream");
            const counts = (
              await git(
                privateRepo,
                "rev-list",
        ...
        if (base.remote && base.ref) {
          try {
            const remoteUrl = await git(item.source, "remote", "get-url", base.remote);
            await git(
              privateRepo,
              "fetch",
              "--no-tags",
            ...
            base.oid = await git(privateRepo, "rev-parse", "refs/finish/base");
            const shallow = await git(privateRepo, "rev-parse", "--is-shallow-repository");
...
export async function discoverFinishRoot(invocation: string): Promise<string> {
  ...
  while (true) {
    try {
      const common = await git(cursor, "rev-parse", "--git-common-dir");
    ...
    .action(
      async (
        ...
        try {
          ...
          if (target) {
            if (isAbsolute(target) || target.startsWith(".") || target.includes(sep))
          ...
          if (report.readiness === "unknown") {
            ...
            for (const repo of report.repositories.filter(
              (entry) => entry.integration === "unknown",
            )) {
              const confirmation = await confirm(
                `Manually confirm ${repo.repository} completed into ${repo.base.remote}:${repo.base.ref}?`,
                false,
              );
              if (confirmation.status !== "ok" || !confirmation.value)
                return done(2, report, "CONFIRMATION_DECLINED");
          ...
          return done(1, undefined, "FINISH_INSPECTION_FAILED");
        }
      },
    );
}
